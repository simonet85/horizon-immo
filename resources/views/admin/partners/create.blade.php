        ...
        <form action="{{ isset($partner) ? route('admin.partners.update', $partner) : route('admin.partners.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="logo" class="block text-sm font-medium text-gray-700 mb-2">
                        Logo <span class="text-red-500">{{ isset($partner) ? '' : '*' }}</span>
                    </label>
                    <input type="file" 
                           id="logo" 
                           name="logo" 
                    ...
                    @if(isset($partner) && $partner->logo)
                        <div class="mt-2">
                            <img src="{{ asset('storage/' . $partner->logo) }}" alt="Logo actuel" class="w-24 h-16 object-contain bg-gray-100 rounded">
            ...
            <div class="mt-6">
                ...
                <textarea id="description" 
                          ...
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $partner->description ?? '') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                <div class="flex items-center">
                    ...
                    <input type="checkbox" 
                           ...
                           name="active" 
                           value="1"
                           {{ old('active', $partner->active ?? true) ? 'checked' : '' }}
            ...
            <div class="mt-8 p-4 bg-gray-50 rounded-lg">
                ...
                <div class="flex flex-col items-center text-center p-6 border border-gray-200 rounded-lg bg-white">
                    ...
                    <h4 id="preview-name" class="text-lg font-semibold text-gray-900 mb-2">{{ old('name', $partner->name ?? 'Nom du partenaire') }}</h4>
                    <p id="preview-description" class="text-sm text-gray-600 mb-2">{{ old('description', $partner->description ?? '') }}</p>
                    <a id="preview-website" href="#" class="text-blue-600 hover:text-blue-800 text-sm" style="{{ old('website_url', $partner->website_url ?? '') ? '' : 'display: none;' }}">
                        Visiter le site
                    </a>
                </div>
            </div>
            ...
            <div class="flex justify-end space-x-4 mt-8">
                ...
                <button type="submit" 
                    ...
                    {{ isset($partner) ? 'Mettre à jour' : 'Créer' }} le partenaire
                </button>
            </div>
        </form>
    </div>

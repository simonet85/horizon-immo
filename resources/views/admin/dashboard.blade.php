        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-sm text-green-600">{{ $stats['available_properties'] }} disponibles</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-sm text-blue-600">{{ $stats['client_users'] }} clients</p>
                </div>
            </div>
        </div>
                ...
                @forelse($recentProperties as $property)
                    <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                        <div class="flex-shrink-0">
                            @if($property->image)
                                <img src="{{ Storage::url($property->image) }}" alt="{{ $property->title }}" class="w-12 h-12 rounded-lg object-cover">
                            @else
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-3">
                @forelse($recentMessages as $message)
                    <div class="flex items-start p-3 bg-gray-50 rounded-lg">
                        <div class="flex-shrink-0">
                            <div class="w-10 h-10 {{ $message->type === 'contact' ? 'bg-blue-100' : 'bg-green-100' }} rounded-full flex items-center justify-center">
                                <span class="{{ $message->type === 'contact' ? 'text-blue-600' : 'text-green-600' }} font-medium text-sm">
                                    @if($message->type === 'contact')
                                        {{ substr($message->name ?? 'U', 0, 1) }}
                                    @else
                                        {{ substr($message->name ?? ($message->user->name ?? 'U'), 0, 1) }}
                                    @endif
                                </span>
                            </div>
                        </div>
                        <div class="ml-3 flex-1">
                            <div class="flex items-center justify-between">
                                <p class="text-sm font-medium text-gray-900">
                                    @if($message->type === 'contact')
                                        {{ $message->name ?? 'Contact anonyme' }}
                                    @else
                                        {{ $message->name ?? $message->user->name ?? 'Utilisateur' }}
                                    @endif
                                </p>
                                <span class="text-xs px-2 py-1 rounded-full {{ $message->type === 'contact' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600' }}">
                                    {{ $message->type === 'contact' ? 'Contact' : 'Propriété' }}
                                </span>
                            </div>
                            <p class="text-sm text-gray-500">{{ Str::limit($message->message, 50) }}</p>
                            @if($message->type === 'property' && $message->property)
                                <p class="text-xs text-gray-400">{{ $message->property->title }}</p>
                            @endif
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.applications.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    Voir tout
                </a>
            </div>
            <div class="space-y-3">
                @forelse($recentApplications as $application)
                    <div class="flex items-start p-3 bg-gray-50 rounded-lg">
                        ...
                        <div class="ml-3 flex-1">
                            ...
                            <p class="text-sm text-gray-500">{{ $application->budget_range }}</p>
...
const activityChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [
            {
                label: 'Propriétés',
                data: @json($monthlyStats['properties']),
            ...
            {
                label: 'Messages',
                data: @json($monthlyStats['messages']),

...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">Modifier la ville</h1>
        ...
        <form action="{{ route('admin.towns.update', $town) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description"
                                  ...
                                  placeholder="Description de la ville...">{{ old('description', $town->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            Seules les villes actives sont visibles sur le site.
                        </p>
                        @error('is_active')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="space-y-4">
                        <div class="bg-white p-3 rounded border">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-lg font-bold text-indigo-600">{{ $town->properties->count() }}</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-white p-3 rounded border">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm text-gray-900">{{ $town->created_at->format('d/m/Y') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-white p-3 rounded border">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm text-gray-900">{{ $town->updated_at->format('d/m/Y') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="space-y-2">
                            @foreach($town->properties->take(3) as $property)
                            <div class="bg-white p-2 rounded border text-xs">
                                <div class="font-medium text-gray-900">{{ Str::limit($property->title, 30) }}</div>
                                <div class="text-gray-500">{{ number_format($property->price, 0, ',', ' ') }} FCFA</div>
                            </div>
                            @endforeach
                            @if($town->properties->count() > 3)
                            <p class="text-xs text-gray-500 text-center">
                                et {{ $town->properties->count() - 3 }} autre(s)...
                            </p>
                            @endif
                        </div>
                    </div>
        ...
        <div class="flex justify-end mt-4 pt-4 border-t border-gray-200">
            <form action="{{ route('admin.towns.destroy', $town) }}"
                  method="POST"
                  onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette ville ? Cette action est irréversible.')">
                @csrf
                @method('DELETE')
                <button type="submit"
                        class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-700 active:bg-red-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150"
                        {{ $town->properties->count() > 0 ? 'disabled title=Cette ville contient des propriétés et ne peut pas être supprimée' : '' }}>
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">

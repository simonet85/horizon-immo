        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->first_name }} {{ $application->last_name }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Email</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->email }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Pays de résidence</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->country_residence }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Âge</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->age }} ans</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Profession</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->profession }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Téléphone</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->phone }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->desired_city }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Type de bien</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->property_type }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Budget</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->budget_range }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Apport personnel</label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->personal_contribution_percentage }}%</p>
                    </div>
                </div>
            </div>

            <!-- Informations complémentaires -->
            @if($application->additional_info || $application->message)
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    @if($application->additional_info)
                        <div class="mb-4">
                            ...
                            <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-md">
                                <p class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{{ $application->additional_info }}</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($application->message)
                        <div>
                            ...
                            <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-md">
                                <p class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{{ $application->message }}</p>
                            </div>
                        </div>
                    @endif
                </div>
            ...
            @if($application->admin_response)
                <div class="p-6">
                    ...
                    <div class="p-4 rounded-md {{ $application->status === 'approved' ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20' }}">
                        <p class="text-sm {{ $application->status === 'approved' ? 'text-green-900 dark:text-green-100' : 'text-red-900 dark:text-red-100' }} whitespace-pre-wrap">{{ $application->admin_response }}</p>
                        @if($application->updated_at != $application->created_at)
                            <p class="text-xs {{ $application->status === 'approved' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }} mt-2">
                                Répondu le {{ $application->updated_at->format('d/m/Y à H:i') }}
                            </p>
                        @endif
                    </div>
                </div>
            ...
            <div class="space-y-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $application->created_at->format('d/m/Y à H:i') }}</p>
                    </div>
                </div>

                @if($application->updated_at != $application->created_at && $application->admin_response)
                    <div class="flex items-start">
                        <div class="flex-shrink-0">
                            <div class="w-8 h-8 {{ $application->status === 'approved' ? 'bg-green-100' : 'bg-red-100' }} rounded-full flex items-center justify-center">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                Demande {{ $application->status === 'approved' ? 'approuvée' : 'refusée' }}
                            </p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $application->updated_at->format('d/m/Y à H:i') }}</p>
                        </div>
                    </div>
                @endif
            </div>

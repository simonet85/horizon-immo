...
@section('title', 'Message sur propriété')
                    ...
                    <div class="border-b border-gray-200 pb-6 mb-6">
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="text-gray-600 mt-1">Envoyé le {{ $message->created_at->format('d/m/Y à H:i') }}</p>
                            </div>
                    ...
                    @if($message->property)
                        <div class="bg-blue-50 rounded-lg p-6 mb-6">
                            ...
                            <div class="flex items-start space-x-4">
                                @if($message->property->images && count($message->property->images) > 0)
                                    <img src="{{ Storage::url($message->property->images[0]) }}" 
                                         alt="{{ $message->property->title }}"
                                         class="w-24 h-24 object-cover rounded-lg">
                                @endif
                                <div class="flex-1">
                                    <h4 class="font-semibold text-gray-900">{{ $message->property->title }}</h4>
                                    <p class="text-gray-600 mt-1">{{ $message->property->address }}</p>
                                    <p class="text-lg font-semibold text-blue-600 mt-2">
                                        {{ number_format($message->property->price, 0, ',', ' ') }} €
                                    </p>
                                    <a href="{{ route('property.detail', $message->property->id) }}" 
